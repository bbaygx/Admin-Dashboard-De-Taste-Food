...
const Add = () => {
  ...
  const generateImageFromCloudinary = async () => {
        ...
        try{
            const response = await axios.post('https://api.cloudinary.com/v1_1/diupvvswt/image/upload', formData).then(res => {
                if (res.status === 200) {
  ...
  const sendDataToServer = async (e) => {
    e.preventDefault();
     ...
     try{
        const req = await axios.post(`${import.meta.env.VITE_BASEURL}/restaurant`, {
          foodName: namaOutlet,
  ...
  return (
    <>
      <form className="container xl:max-w-md m-auto py-32" onSubmit={sendDataToServer}>
        <h3 className="py-6 text-2xl font-semibold">Add Outlet</h3>
        <div className="mb-6">
          <div>
            ...
            <div className="mt-1 text-sm text-gray-500 dark:text-gray-300" id="user_avatar_help">
              Kirim dulu gambarnya ke server soalnya kita mau dapat URL gambarnya
            </div>
            <span className="btn btn-neutral" id='files' onClick={generateImageFromCloudinary}>Send</span>
          </div>
        </div>
        <div className="mb-6">
